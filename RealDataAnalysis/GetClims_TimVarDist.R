## Obtains the control limits using the time varying CDFs to standarndize (opposed to using the stationary dist to standardize)
...
make_method_Biv8_TimeVar <- function(p1, priorParams, rhoVec, cdfsList, dist_map) {
  list(
    name = "Biv8_CUSUM_TimeVar",
    rl = function(maxIter, h) {
      Biv8_CUSUM_TimeVar(maxIter = maxIter, h = h, p1 = p1, priorParams = priorParams, rhoVec = rhoVec, cdfsList = cdfsList, dist_map = dist_map)
    }
  )
}
make_ARL_policy <- function(targetARL0, fewerIter = FALSE) {
  ...
  } else if (targetARL0 >= 400 && targetARL0 <= 449) {
    if (fewerIter) {
      Ns <- c(200, 250)
      eps <- c(300, 250)
    } else {
  ...
  } else if (targetARL0 >= 66 && targetARL0 <= 98) {
    if (fewerIter) {
      Ns <- c(100, 200)
      eps <- c(10, 9)
    } else {
...
make_clims_job_Biv8_TimeVar <- function(row, TimeVar_cdfs, dist_map, rhoVec, h_init, fewerIter = TRUE, logfile = NULL, logger = NULL) {
  
  method <- make_method_Biv8_TimeVar(
    p1          = row$p1,
    priorParams = c(row$prior1, row$prior2, row$prior3, row$prior4),
    rhoVec      = rhoVec,
    cdfsList    = TimeVar_cdfs,
    dist_map    = dist_map
  )
  ...
  job$rho1      <- row$rho1
  job$rho2      <- row$rho2
  job$methodName <- "Biv8_CUSUM_TimeVar"
...
########### Main functions ###########
Biv8_CUSUM_TimeVar <- function(maxIter, h, p1, priorParams, rhoVec, cdfsList, dist_map)   {
...
####### Main #######
setwd("/path/to/folder/RealDataAnalysis/") # Set to your working directory

########### Load Stable Distributions ###############
# Given working directory, this is where we expect the stable distribution to be stored
...
snapshot_times <- c(1:20, 25, seq(30, 100, by = 10), seq(150, 400, by = 50))
snapshot_times
...
prior_df <- rbind(
  ...
  # make_prior_df(priorsMat2, rhoVals2, "P2")
)
...
clims_file <- file.path(clims_dir, "Clims_CUSUM_TimeVar.txt")
...
results <- parLapply(cl, jobs, function(row) {
  logfile <- file.path(IC_rls_dir, IC_Rl_fname(row))

  job <- make_clims_job_Biv8_TimeVar(
    row         = row,
    TimeVar_cdfs = TimeVar_cdfs,
    dist_map    = dist_map,
